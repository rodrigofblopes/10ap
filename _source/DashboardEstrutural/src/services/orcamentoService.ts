...
export const carregarDados = async (): Promise<OrcamentoItem[]> => {
  try {
    // Carregar apenas o arquivo 5DEST.csv (dados reais)
    console.log('🚀 ===== INICIANDO CARREGAMENTO DE DADOS =====');
    console.log('📁 Carregando dados reais da planilha 5DEST.csv...');
    ...
    const possiblePaths = ['/5DEST.csv', './5DEST.csv', '5DEST.csv'];
    ...
    if (!csvResponse || !csvResponse.ok) {
      throw new Error(`Erro ao carregar 5DEST.csv de qualquer caminho. Status: ${csvResponse?.status}`);
    }
    ...
    const dadosProcessados = processarDadosCSV5DEST(csvContent);
...
// CORREÇÃO COMPLETA DO PARSING CSV - 5DEST.csv
export const processarDadosCSV5DEST = (csvContent: string): OrcamentoItem[] => {
  console.log('📊 ===== PROCESSANDO CSV 5DEST (VERSÃO CORRIGIDA) =====');
  console.log('📄 Processando CSV 5DEST...');
